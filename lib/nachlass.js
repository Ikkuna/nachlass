  ...
  exports.client.send_command('FT.INFO',[key],function(err,info) {
    if (err) { 
      ...
      if (String(err).indexOf('Unknown Index name') > 0 ){
        ...
        const fields_validation = Array.from( fields_entries ).reduce( (acc, x, i) => acc.concat(Array.from(x)), [] )   // dirty hacking until flat() is implemented
        console.log('fields_validation',fields_validation,'is array' , Array.isArray(fields_validation) ) ;
        const schema_inyection =  fields_validation.join(' ');// fields_validation.flat(1).join(' '); flat is not implemented yet
        console.log( 'schema_inyection : ',schema_inyection );
        let args = [
          key,
          'SCHEMA', schema_inyection //,  'text'   // there's another syntax for aggregate
        ];
...
function Search(key,info,opts) {
  ...
  Search.prototype.index = function(str, id, fn){
    ...
    if ( Object.keys(opts.payloadField).length !== tags.length ) console.log('SCHEMA and DATA not compatible'); else console.log('SCHEMA OK')
    db.send_command(
      'FT.ADD',
      [
        key,
      ...
      .concat( tags.flat(1).join(' ') )  // pareamos FIELDS con strings. idealmente esto dependería sólo de 'str'.
      , fn || noop 
    );
