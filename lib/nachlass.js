  ...
  exports.client.send_command('FT.INFO',[key],function(err,info) {
    if (err) { 
      ...
      if (String(err).indexOf('Unknown Index name') > 0 ){
        const fields_entries = Object.entries(opts.fields)
        console.log('fields entries', JSON.stringify(fields_entries))
        const fields_validation = JSON.parse( JSON.stringify(fields_entries) )
        console.log('fields' , typeof fields_validation ) ;
