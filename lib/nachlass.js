  ...
  exports.client.send_command('FT.INFO',[key],function(err,info) {
    if (err) { 
      ...
      if (String(err).indexOf('Unknown Index name') > 0 ){
        let args = [
          key,
          'SCHEMA', opts.payloadField, 'text'   // there's another syntax for aggregate
        ];
...
function Search(key,info,opts) {
  ...
  Search.prototype.index = function(str, id, fn){
    ...
    var db = this.client;
    var opts = this.opts;
    if (Object.entries(opts.payloadField).length !== str.split(' ').length ) console.log('SCHEMA and String not compatible'); else console.log('SCHEMA OK')
    db.send_command(
      'FT.ADD',
      [
        key,
      ...
      .concat( Object.keys(opts.payloadField).map( (x,i) => [x].concat( str.split(' ')[i] ) ).flat(1) )  // pareamos FIELDS con strings. idealmente esto dependería sólo de 'str'.
      , fn || noop 
    );
