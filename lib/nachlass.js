/*!
 * redredisearch
 * 
 * Forked from tj/reds
 * Original work Copyright(c) 2011 TJ Holowaychuk <tj@vision-media.ca>
 * Modified work Copyright(c) 2017 Kyle Davis
 * MIT Licensed
 */

/**
 * Module dependencies.
 */

/** For extending library */

const flatter = (toFlat) => toFlat.reduce( (acc, x, i) => Array.isArray(x[1]) ? acc.concat(flatter(x)) /*flatter(x)*/ : acc.concat(x), [] )
const lang = 'spanish' // default language

/** END of extension library */


var redis = require('redis');
function noop(){};

/**
 * Library version.
 */

exports.version = '0.0.1';

/**
 * Expose `Search`.
 */

exports.Search = Search;

/**
 * Expose `Query`.
 */

exports.Query = Query;

/**
 * Expose `Aggregate`.
 */

exports.Aggregate = Aggregate;

/**
 * Search types.
 */

var types = {
  intersect: 'and',
  union: 'or',
  and: 'and',
  or: 'or'
};

/**
 * Alternate way to set client 
 * provide your own behaviour.
 *
 * @param {RedisClient} inClient
 * @return {RedisClient}
 * @api public
 */

exports.setClient = function(inClient) {
  return exports.client = inClient;
}

/**
 * Create a redis client, override to
 * provide your own behaviour.
 *
 * @return {RedisClient}
 * @api public
 */

exports.createClient = function(){
  return exports.client
    || (exports.client = redis.createClient());
};

/**
 * Confirm the existence of the RediSearch Redis module
 *
 * @api public
 */

exports.confirmModule = function(cb) {
  exports.client.send_command('ft.create',[], function(err) {
    let strMsg = String(err);
    if (strMsg.indexOf('ERR wrong number of arguments') > 0) {
      cb(null);
    } else {
      cb(err);
    }
  });
}

/**
 * Return a new reds `Search` with the given `key`.
 * @param {String} key
 * @param {Object} opts
 * @return {Search}
 * @api public
 */

exports.createSearch = function(key,opts,cb){
  const 
    searchObj   = function(err,info) {
      if (err) { cb(err); } else {
        cb(err,new Search(key,info,opts), new Aggregate(key,info,opts));
      }
    };

  opts = !opts ? {} : opts;
  opts.payloadField = opts.payloadField ? opts.payloadField : 'payload TEXT';

  if (!key) throw new Error('createSearch() requires a redis key for namespacing');
  
  exports.client.send_command('FT.INFO',[key],function(err,info) {
    if (err) { 
      //if the index is not found, we need to make it.
      if (String(err).indexOf('Unknown Index name') > 0 ){
        // const fields_entries = JSON.stringify( Object.entries(opts.fields) )
        // console.log('fields entries', fields_entries, 'type', typeof fields_entries)
        // const flatter = (toFlat) => Object.entries(toFlat).reduce( (acc, x, i) => Array.isArray(x) ? flatter(x) : acc.concat(x), [] )
        // const fields_validation = flatter(opts.fields) //Object.entries(opts.fields).reduce( (acc, x, i) => acc.concat(x), [] )   // dirty hacking until flat() is implemented
        // const flatter = (toFlat) => toFlat.reduce( (acc, x, i) => Array.isArray(x[1]) ? acc.concat(flatter(x)) /*flatter(x)*/ : acc.concat(x), [] )
        const fields_validation = flatter(Object.entries(opts.fields))
        console.log('fields_validation',fields_validation,'is array' , Array.isArray(fields_validation) ) ;
        const schema_inyection =  fields_validation;// fields_validation.flat(1).join(' '); flat is not implemented yet
        console.log( 'schema_inyection : ',schema_inyection );
        const stopwords_validation = opts.stopwords ? flatter(Object.entries(opts.stopwords)) : [] ;
        let args = [key]
          .concat(['STOPWORDS']).concat([stopwords_validation.length]).concat(stopwords_validation)   // cannot use stopwords if they are removing numbers and special characters ***
          .concat(['SCHEMA']).concat(schema_inyection);  //,  'text'   // there's another syntax for aggregate
        exports.client.send_command(
          'FT.CREATE',
          args,
          function(err) {
            if (err) { cb(err); } else {
              exports.client.send_command('FT.INFO',[key],searchObj);
            }
          }
        );
      }

    } else { searchObj(err,info); }
  });
};

/**
 * Return the words in `str`. This is for compatability reasons (convert OR queries to pipes)
 *
 * @param {String} str
 * @return {Array}
 * @api private
 */

exports.words = function(str){
  return String(str).match(/\w+/g);
};


/**
 * Initialize a new `Query` with the given `str`
 * and `search` instance.
 *
 * @param {String} str
 * @param {Search} search
 * @api public
 */

function Query(str, search, queryOpts) {
  this.str = str;
  this.type('and');
  this.search = search;
  this.queryOpts = queryOpts;
}

      /**
       * Set `type` to "union" or "intersect", aliased as
       * "or" and "and".
       *
       * @param {String} type
       * @return {Query} for chaining
       * @api public
       */

      Query.prototype.type = function(type){
        if (type === 'direct') {
          this._directQuery = true;
        } else {
          this._directQuery = false;
          this._type = types[type];
        }
        return this;
      };

      /**
       * Limit search to the specified range of elements.
       *
       * @param {String} start
       * @param {String} stop
       * @return {Query} for chaining
       * @api public
       */
      Query.prototype.between = function(start, stop){
        this._start = start;
        this._stop = stop;
        return this;
      };

      /**
       * Configure Phonetics.
       *
       * @param {String} phonetic
       * @return {Query} for chaining
       * @api public
       */
      Query.prototype.phonetic = function(phonetic){
        this._phonetic = phonetic;
        return this;
      };

      /**
       * Perform the query and callback `fn(err, ids)`.
       *
       * @param {Function} fn
       * @return {Query} for chaining
       * @api public
       */

      Query.prototype.end = function(fn){
        var 
          key     = this.search.key,
          db      = this.search.client,
          query   = this.str,
          direct  = this._directQuery,
          phonetic = this._phonetic,
          args    = [],
          joiner  = ' '/*,
          rediSearchQuery;*/

        const queryOpts_validation = flatter(Object.entries(this.queryOpts));

        const rediSearchQuery = (type) =>
          type ?
            () => {
              const parsedQuery = Array.isArray(query) ? [].concat(...query).join(joiner) : query
              return phonetic == false ? '('+parsedQuery+')=>{$phonetic:'+phonetic+'}' : query;
            }
            :
            () => {
              rediSearchQuery = exports.words(query);
              return this._type === or ?
                rediSearchQuery.join('|')
                :
                rediSearchQuery.join(joiner);
            }

        // if (direct) {
        //     const parsedQuery = typeof query == String ? query : [].concat(...query)
        //     rediSearchQuery = phonetic == false ? '"('+( () => Array.isArray(parsedQuery) ? parsedQuery.join(joiner) : parsedQuery)().replace(/[!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~]/g, '\\$&')+')=>{$phonetic:'+phonetic+'}"' : query;
        //     console.log('rediSearchQuery', rediSearchQuery)
        // } else {
        //   rediSearchQuery = exports.words(query);
        //   if (this._type === 'or') {
        //     joiner = '|'
        //   }
        //   rediSearchQuery = rediSearchQuery.join(joiner);
        // } 
        args = [
          key,
          rediSearchQuery(direct)()/*,
          'NOCONTENT'*/
        ]
        .concat(queryOpts_validation);

        if (this._start) {
          args.push('LIMIT',this._start,this._stop - this._start);
        }

        // console.log('[nachlass] Query args', args)

        db.send_command(
          'FT.SEARCH',
          args,
          function(err,resp) {
            if (err) { fn(err); } else {
              fn(err,resp/*.slice(1)*/);
            }
          }
        );

        return this;
      };


// Inserting Syntax for Aggregate Calls to Redisearch  *******************************************


function Aggregate(str, search, queryOpts) {
  this.str = str;
  this.type('direct');
  this.search = search;
  this.queryOpts = queryOpts;
}

      /**
       * Set `type` to "union" or "intersect", aliased as
       * "or" and "and".
       *
       * @param {String} type
       * @return {Query} for chaining
       * @api public
       */

      Aggregate.prototype.type = function(type){
        if (type === 'direct') {
          this._directQuery = true;
        } else {
          this._directQuery = false;
          this._type = types[type];
        }
        return this;
      };

      /**
       * Limit search to the specified range of elements.
       *
       * @param {String} start
       * @param {String} stop
       * @return {Query} for chaining
       * @api public
       */
      Aggregate.prototype.between = function(start, stop){
        this._start = start;
        this._stop = stop;
        return this;
      };

      /**
       * Perform the query and callback `fn(err, ids)`.
       *
       * @param {Function} fn
       * @return {Query} for chaining
       * @api public
       */

      Aggregate.prototype.end = function(fn){
        var 
          key     = this.search.key,
          db      = this.search.client,
          query   = this.str,
          direct  = this._directQuery,
          args    = [],
          joiner  = ' ',
          rediSearchQuery;

          if (direct) {
          rediSearchQuery = query;
        } else {
          rediSearchQuery = exports.words(query);
          if (this._type === 'or') {
            joiner = '|'
          }
          rediSearchQuery = rediSearchQuery.join(joiner);
        } 
        args = [
          key,
          rediSearchQuery
        ];

        if (this._start) {
          args.push('LIMIT',this._start,this._stop - this._start);
        }

        db.send_command(
          'FT.AGGREGATE',
          args,
          function(err,resp) {
            if (err) { fn(err); } else {
              fn(err,resp.slice(1));
            }
          }
        );

        return this;
      };

  // FT.AGGREGATE
  // {index_name:string}
  // {query_string:string}
  // [WITHSCHEMA] [VERBATIM]
  // [LOAD {nargs:integer} {property:string} ...]
  // [GROUPBY
  //   {nargs:integer} {property:string} ...
  //   REDUCE
  //     {FUNC:string}
  //     {nargs:integer} {arg:string} ...
  //     [AS {name:string}]
  //   ...
  // ] ...
  // [SORTBY
  //   {nargs:integer} {string} ...
  //   [MAX {num:integer}] ...
  // ] ...
  // [APPLY
  //   {EXPR:string}
  //   AS {name:string}
  // ] ...
  // [FILTER {EXPR:string}] ...
  // [LIMIT {offset:integer} {num:integer} ] ...




// End of Aggregate Calls Insertion  ********************************************************

/**
 * Initialize a new `Suggestion` with the given `key`.
 *  
 * @param {String} key
 * @param {Object} opts
 * @api public
 */
var Suggestion = function(key,opts) {
  this.key = key;
  this.client = exports.createClient();
  this.opts = opts || {};
  if (this.opts.fuzzy) {
    this.fuzzy = opts.fuzzy;
  }
  if (this.opts.maxResults) {
    this.maxResults = opts.maxResults;
  }
  if (this.opts.incr) {
    this.incr = opts.incr;
  }
  if (this.opts.withPayloads) {
    this.withPayloads = true;
  }
}

/**
 * Create a new Suggestion object
 * 
 * @param {String} key
 * @param {Object} opts
 * @api public 
 */
exports.suggestionList = function(key,opts) {
  return new Suggestion(key,opts);
}

/**
 * Set `fuzzy` on suggestion get. Can also be set via opts in the constructor
 *
 * @param {Boolean} isFuzzy
 * @return {Suggestion} for chaining
 * @api public
 */

Suggestion.prototype.fuzzy = function(isFuzzy){
  this.fuzzy = isFuzzy;
  return this;
};

/**
 * Set the max number of returned suggestions. Can also be set via opts in the constructor
 *
 * @param {Number} maxResults
 * @return {Suggestion} for chaining
 * @api public
 */

Suggestion.prototype.maxResults = function(maxResults){
  this.maxResults = maxResults;
  return this;
};

Suggestion.prototype.add = function(str,score,payload,fn) {
  if((typeof fn === 'undefined' || fn === null) && typeof payload === "function"){
    if(typeof fn !== 'undefined'){
      fn = payload;
    } else {
      var fn = payload;
    }
    payload = null;
  };

  var key = this.key;
  var db = this.client;
  var args = [
    key,
    str,
    score,
  ];
  if (this.incr) {
    args.push('INCR');
  }
  if(payload !== null){
    args.push('PAYLOAD', (typeof payload === 'object' ? JSON.stringify(payload) : payload.toString()));
  }
  db.send_command(
    'FT.SUGADD',
    args,
    fn || noop
  );
  return this;
}

Suggestion.prototype.get = function(prefix,fn) {
  var key = this.key;
  var db = this.client;
  var args = [
    key,
    prefix
  ];
  if (this.fuzzy) {
    args.push('FUZZY');
  }
  if (this.maxResults) {
    args.push('MAX',this.maxResults);
  }
  if (this.withPayloads) {
    args.push('WITHPAYLOADS');
  }

  db.send_command(
    'FT.SUGGET',
    args,
    fn
  );

  return this;
}

Suggestion.prototype.del = function(str,fn) {
  var key = this.key;
  var db = this.client;

  db.send_command(
    'FT.SUGDEL',
    [ 
      key,
      str
    ],
    fn
  );

  return this;
}

Search.prototype.synadd = function(list, fn){
  var key = this.key;
  var db = this.client;
  // var opts = this.opts;
  // var tags = Object.entries(str)
  // var fields_data = tags.reduce( (acc, x, i) => x[1] !== '' ? acc.concat(x) : acc.concat(x[0],'null'), [] )
  // const score = this.opts.score ? this.opts.score : 1;
  // console.log('fields_data', fields_data)
  var add_args =      [
    key,
    ...list,
    // score,            //default - this should be to be set in future versions
    // 'NOSAVE',     //emulating Reds original behaviour
    // 'REPLACE',    //emulating Reds original behaviour
    // 'FIELDS'
  ]
  // .concat( fields_data )  // pareamos FIELDS con strings. idealmente esto dependería sólo de 'str'.
  // if ( Object.keys(opts.fields).length !== tags.length ) console.log(add_args,'SCHEMA and DATA not compatible'); //else console.log('SCHEMA LENGTH OK');//add_args,'SCHEMA LENGTH OK')
  db.send_command(
    'FT.SYNADD',
    add_args
    , fn || noop 
  );

  return this;
};

/**
 * Initialize a new `Search` with the given `key`.
 *
 * @param {String} key
 * @api public
 */

function Search(key,info,opts) {
  this.key = key;
  this.client = exports.createClient();
  this.opts = opts;
}

  /**
   * Index the given `str` mapped to `id`.
   *
   * @param {String} str
   * @param {Number|String} id
   * @param {Function} fn
   * @api public
   */

  Search.prototype.index = function(str, id, fn){
    var key = this.key;
    var db = this.client;
    var opts = this.opts;
    var tags = Object.entries(str)
    var fields_data = tags.reduce( (acc, x, i) => x[1] !== '' ? acc.concat(x) : acc.concat(x[0],'null'), [] )
    const score = this.opts.score ? this.opts.score : 1;
    const language = this.opts.language ? this.opts.language : lang;
    // console.log('fields_data', fields_data)
    var add_args =      [
      key,
      id,
      score,            //default - this should be to be set in future versions
      // 'NOSAVE',     //emulating Reds original behaviour
      'REPLACE',    //emulating Reds original behaviour
      'FIELDS'
    ]
    .concat( fields_data )  // pareamos FIELDS con strings. idealmente esto dependería sólo de 'str'.
    .concat( ['LANGUAGE' , language ] )
    if ( Object.keys(opts.fields).length !== tags.length ) console.log(add_args,'SCHEMA and DATA not compatible'); //else console.log('SCHEMA LENGTH OK');//add_args,'SCHEMA LENGTH OK')
    db.send_command(
      'FT.ADD',
      add_args
      , fn || noop 
    );

    return this;
  };

  /**
   * Remove occurrences of `id` from the index.
   *
   * @param {Number|String} id
   * @api public
   */

  Search.prototype.remove = function(id, fn){
    fn = fn || noop;
    var key = this.key;
    var db = this.client;
    
    //this.removeIndex(db, id, key, fn);
    db.send_command(
      'FT.DEL',
      [
        key,
        id
      ],
      fn
    )
    
    return this;
  };

  /**
   * Perform a search on the given `query` returning
   * a `Query` instance.
   *
   * @param {String} query
   * @param {Query}
   * @api public
   */

  Search.prototype.query = function(query, queryOpts){
    return new Query(query, this, queryOpts);
  };

  Aggregate.prototype.query = function(query, queryOpts){
    return new Aggregate(query, this, queryOpts);
  };